...
function html2md(str: string, options?: Html2MdOptions, force = false): string {
  ...
  if (options?.enableATF==true && typeof options.extraData == 'object'){
    enableATF = true;
  ...
  while (nextTagStr !== '') {
    if (nextTagName != null) {
      ...
      const options = {
        ...
        prevTagName,
        prevTagStr: res,
        extraData: (enableATF) ? extraData : undefined,
        picNoWrap: false,
