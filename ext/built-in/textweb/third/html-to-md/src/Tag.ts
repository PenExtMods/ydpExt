...
import { AFTExtraData, ParseOptions, TagName, TagOptions, TagProps } from './type'
...
class Tag implements TagProps {
  ...
  innerHTML: string
  extraData: AFTExtraData | undefined
  picNoWrap: boolean
  ...
  parseOnlyString(
    ...
    // fix no escape for '|' in table
    if (this.inTable) return rawString.exec().replace(/[\n\r]/g,' ').replace(/(?<=[^\\])[\|]/g,'\\|');
  ...
  exec(prevGap = '', endGap = '') {
    // fix in table
    if (this.inTable && this.tagName!='tr' && this.tagName!='thead' && this.tagName!='tbody'){
      if (prevGap.trim().length==0) prevGap = '';
      if (endGap.trim().length==0) endGap = '';
    }
